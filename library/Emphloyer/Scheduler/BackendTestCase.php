...
class BackendTestCase extends \PHPUnit_Framework_TestCase {
  ...
  public function testGetAllEntries() {
    ...
    foreach ($schedules as $idx => $schedule) {
      ...
      $job->setName('Job ' . $idx);
      $scheduled[] = $this->scheduler->schedule($job, $schedule[0], $schedule[1], $schedule[2], $schedule[3], $schedule[4]);
    }
  ...
  public function testSchedulingVariations() {
    ...
    foreach ($schedules as $idx => $schedule) {
      $job = new BackendTestJob();
      $job->setName('Job ' . $idx);
      $entry = $this->scheduler->schedule($job, $schedule[0], $schedule[1], $schedule[2], $schedule[3], $schedule[4]);
      $this->assertInstanceOf("Emphloyer\Scheduler\ScheduleEntry", $entry);
